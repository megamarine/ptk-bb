...
if(isset($_GET["seq"]) or isset($_POST["simpan"]))
{
    ...
    if($BETUL > 0)
    {
        ...
        $mail_subject = "PTK Online - Director Approval (Direct) for " . $seq;
        $trans_type   = "Approval Director (Direct)";
        // EMAIL Recruitment
    ...
    // JIKA USER ADALAH DIRECTOR BIASA
    else
    {
        $result  = $db1->prepare("update t_ptk set app_dir = '1', app_dir_date = '$DATE' where seq = '$seq'");
        $result->execute();
        $HEADER       = "<h4> Dear HRD Manager,</h4>
                         <p>This is auto generate email from PTK Online System that asking approval for below employee request :</p>";
        $mail_subject = "PTK Online - Director Approval for " . $seq;
        $trans_type   = "Approval Director";
        $EMAIL_RCR    = "";
    }
