...
if(isset($_GET["seq"]) or isset($_POST["simpan"]))
{
    $DATE          = date("Y-m-d");
    $seq           = isset($_GET["seq"]) ? $_GET["seq"] : $_POST["seq"];
    ...
    $TYPE_CONTRACT = isset($_POST["TYPE_CONTRACT"]) ? $_POST["TYPE_CONTRACT"] : null;

    // CEK APAKAH USER APPROVAL MGR == USER APPROVAL MD -> BU HISAKO & PAK RACHMAT
    ...
    // JIKA BETUL USER ADALAH MD
    if($BETUL > 0)
    {
        $result  = $db1->prepare("update t_ptk 
                                     set app_mgr = '1', 
                                         app_mgr_date = '$DATE', 
                                         app_dir = '1', 
                                         app_dir_date = '$DATE', 
                                   ...
                                   where seq = '$seq'");
        $result->execute();
        $HEADER       = "<h4> Dear All,</h4>
                         <p>This is auto generate email from PTK Online System that inform you about approval for below employee request :</p>";
        $mail_subject = "PTK Online - Manager Approval (Direct) for " . $seq;
        $trans_type   = "Approval Manager (Direct)";
        ...
        // EMAIL RCR
        $query5 = $db1->prepare("select email from m_user where akses = 'Recruitment' and status='aktif'");
        $query5->execute();
        while ($row_query5 = $query5->fetch(PDO::FETCH_ASSOC))
        {
            $EMAIL_RCR = $row_query5["email"];
        }
    }
    // JIKA USER ADALAH MANAGER BIASA
    else
    {
        $result  = $db1->prepare("update t_ptk set app_mgr = '1', app_mgr_date = '$DATE' where seq = '$seq'");
        $result->execute();
        $HEADER  = "<h4> Dear Director,</h4>
                    <p>This is auto generate email from PTK Online System that asking approval for below employee request :</p>";
        $mail_subject = "PTK Online - Manager Approval for " . $seq;
        $trans_type   = "Approval Manager";
        $EMAIL_HRD    = "";
        $EMAIL_MD     = "";
        $EMAIL_RCR    = "";
    }
    ...
    if($result)
    {
            ...
            "'', 'Approved - $seq', '$IP_ADDRESS','$ID_USER1','$DINO','$ID_USER1','PTK','$trans_type'");
