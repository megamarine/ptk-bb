...
if(isset($_GET["seq"]) or isset($_POST["simpan"]))
{
    ...
    $result = $db1->prepare("update t_ptk 
                              ...
                              where seq = '$seq'");
    $result->execute();
    $mail_subject ="PTK Online - Managing Director Approval for " . $seq;
    ...
    if($result)
    {
            ...
            "'', 'Approved - $seq, set Type Kontrak = $TYPE_CONTRACT', '$IP_ADDRESS','$ID_USER1','$DINO','$ID_USER1','PTK','Approval MD'");
        ...
        $mail->addAddress($EMAIL_RCR);
        $mail->addCC($EMAIL_MGR);
        $mail->addCC($EMAIL_DIR);
        $mail->addCC($EMAIL_HRD);
        $mail->addCC($EMAIL_MD);
        $mail->Subject = $mail_subject;
        $mail->msgHTML("<h4> Dear Recruitment Team,</h4>
