...
$update_munit = "";
$delete_munit = "";
$query      = getQuery("select * from m_usermodule where kode_user = '$KODE_USER' and id_module = '10' and xread = '1'");
$cek = $query->rowCount();
if($cek == 0)
{
    ?> 
...
else
{
    ...
    {
        $create_munit = $roww["xcreate"];
        $read_munit   = $roww["xread"];
        $update_munit = $roww["xupdate"];
        $delete_munit = $roww["xdelete"];
    }
}
// ---------------------------------------------------------------------------------------------------------------------------------
