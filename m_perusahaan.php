...
$update_mperusahaan = "";
$delete_mperusahaan = "";
$query      = getQuery("select * from m_usermodule where kode_user = '$KODE_USER' and id_module = '3' and xread = '1'");
$cek = $query->rowCount();
if($cek == 0)
{
    ?> 
...
else
{
    ...
    {
        $create_mperusahaan = $roww["xcreate"];
        $read_mperusahaan   = $roww["xread"];
        $update_mperusahaan = $roww["xupdate"];
        $delete_mperusahaan = $roww["xdelete"];
    }
}
// ---------------------------------------------------------------------------------------------------------------------------------
